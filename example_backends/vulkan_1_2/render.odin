...
submit :: proc(b: ^Backend, t: ^Target) {
	...
	si := vk.SubmitInfo {
		...
		pWaitSemaphores      = &f.acquire,
		pWaitDstStageMask    = &wait_stage,
		commandBufferCount   = 1,
		pCommandBuffers      = &f.cmd,
		signalSemaphoreCount = 1,
		pSignalSemaphores    = &t.release[image],
	}
	vk.QueueSubmit(b.queue, 1, &si, f.fence)
...
begin_pass :: proc(b: ^Backend, t: ^Target, f: ^Frame, image: u32) {
	...
	bi := vk.RenderPassBeginInfo {
		...
		renderPass      = b.pass,
		framebuffer     = t.fbs[image],
		renderArea      = {extent = t.extent},
		clearValueCount = 1,
		pClearValues    = &cv,
	}
	vk.CmdBeginRenderPass(f.cmd, &bi, .INLINE)
}
