...
make_target :: proc(b: ^Backend, t: ^Target, win: glfw.WindowHandle) -> bool {
	...
	t.fbs = make([dynamic]vk.Framebuffer, 0, 4)
...
rebuild_target :: proc(
	...
	for img in t.images {
		...
		append(&t.fbs, make_framebuffer(b, t, view))
	}
...
@(private)
make_framebuffer :: proc(b: ^Backend, t: ^Target, view: vk.ImageView) -> vk.Framebuffer {
	views: [2]vk.ImageView
	n: u32
	if b.samples != {._1} {
		views = {t.ms.view, view}
		n = 2
	} else {
		views = {view, 0}
		n = 1
	...
	fb: vk.Framebuffer
	vk.CreateFramebuffer(b.device, &ci, nil, &fb)
	return fb
}
