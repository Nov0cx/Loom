...
load_face :: proc(b: ^Backend, ttf: []byte, spacing: f32 = 0) -> ui.Font {
	assert(b.n_faces < MAX_FACES, "ui_opengl: too many faces")
	f := &b.faces[b.n_faces]
...
new_page :: proc(b: ^Backend) -> int {
	...
	p.tex = make_texture(n, n, raw_data(p.pixels))
	append(&b.pages, p)
...
glyph_of :: proc(b: ^Backend, font: ui.Font, px: int, r: rune) -> Glyph {
	...
	if w > 0 && h > 0 {
		...
		if ok {
			...
			stbtt.MakeCodepointBitmap(&f.info, raw_data(cov), c.int(w), c.int(h), c.int(w), scale, scale, r)
...
@(private)
upload_rect :: proc(p: ^Page, x, y, w, h: int) {
	row := make([]u8, w * 4, context.temp_allocator)
	gl.BindTexture(gl.TEXTURE_2D, p.tex)
	gl.PixelStorei(gl.UNPACK_ALIGNMENT, 1)
	for r in 0 ..< h {
		copy(row, p.pixels[((y + r) * p.w + x) * 4:][:w * 4])
		gl.TexSubImage2D(
			gl.TEXTURE_2D,
			0,
			i32(x),
